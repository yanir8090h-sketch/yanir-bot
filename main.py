...
class TicketOpenView(discord.ui.View):
    ...
    @discord.ui.button(label="פתח טיקט ✉️", style=discord.ButtonStyle.blurple, custom_id="open_ticket_btn")
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        guild = interaction.guild
        member = interaction.user
        overwrites = {
...
class VerifyView(discord.ui.View):
    ...
    @discord.ui.button(label="להתחלת אימות 🛡️", style=discord.ButtonStyle.green, custom_id="verify_user_btn")
    async def verify(self, interaction: discord.Interaction, button: discord.ui.Button):
        guild = interaction.guild
        member = interaction.user
        verified_role = discord.utils.get(guild.roles, name="Verified")
...
async def setup_shop(ctx):
    ...
    embed = discord.Embed(
        ...
        description=f"🛍️ **חנות הרולים של השרת**\n\n👑 <@&{ROLE_SILVER_ID}> — 30,000 XP\n"
                    f"💎 <@&{ROLE_GOLD_ID}> — 20,000 XP\n🔥 <@&{ROLE_DIAMOND_ID}> — 10,000 XP\n"
                    f"⚡ <@&{ROLE_MASTER_ID}> — 5,000 XP\n✨ <@&{ROLE_GOD_ID}> — 2,500 XP",
...
async def apply_staff_friend(ctx):
    ...
    embed.add_field(name="🟡 סטטוס:", value="ממתין לטיפול המנהלים...", inline=False)
    ...
    await ctx.send("✅ בקשתך נשלחה בהצלחה!", ephemeral=True)

# ==========================================
# 5. פקודת העזרה המעוצבת החדשה (!h) עם סיבה דינמית
# ==========================================
@bot.command(name="h")
